...
moderators = ["iMooseMoo","USER._NOT_FOUND","OOILoveYouOO","_00_DEVIL_00","___kastu__28","Zx._QUEEN","amirhassan_","YO_ANGEL","Ironfiest"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("working")
        await self.highrise.walk_to(Position(19.5 , 0.0 , 6.5 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Welcome {user.username} HARYANA ❤️")
